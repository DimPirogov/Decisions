                ...
                switch (meny)
                {
                    ...
                    case 2: // Här ska vi sortera sifferlistan.                                                                       (BUBBELSORTERING AV SIFFROR)
                        ...
                        if (sifferLista.Count > 0)
                        {
                             ...
                             */
                            // vi går igenom alla sifferlistans element minus 1 
                            for (int i = 0; i < sifferLista.Count - 1; i++)
                            {
                                // vi går igenom element per element vid varje omgång av ovan for-loop
                                for (int j = 0; j < ((sifferLista.Count - 1) - i); j++)
                                {
                                    // om siffran till vänster är större än den till höger så ska dessa byta plats
                    ...
                    case 3: // Här ska användaren kunna söka bland siffrorna med en binärsökning.                                     (BINÄRSÖKNING AV SIFFROR)
                        ...
                        if (sifferLista.Count > 0)
                        {
                            if (intSortering)
                            {
                                ...
                                int first = 0;
                                int last = sifferLista.Count - 1;
                                while(first <= last)
                                {
                                    int middle = (first + last) / 2;
                                    if(key > sifferLista[middle])
                                        first = middle + 1;
                                    else if(key < sifferLista[middle])
                                        last = middle - 1;
                                    else
                                    {
                                        // Använver variablerna för att visa upp sökningsvariabeln och platsen
                                        Console.WriteLine("\n\tNummer {0} finns på plats nr {1}?", key, middle + 1);
                                        // jag har lagt till break för att while fortsätter om man söker på ett nummer
                                        // som är direkt i mitten i början då är first < last och while-loop fortsätter för alltid
                                        break;
                                    }
                                }
                                if(first > last)
                                    Console.WriteLine("\n\tSökning på nr {0} misslyckade", key);
                    ...
                    case 5: // Här ska du skriva klart bubbelsorteringsalgoritmen som behövs för att sortera våra nonsensord i ordLista.                                (BUBBELSORTERING AV BOKSTÄVER)
                        ...
                        if (ordLista.Count > 0)
                        {
                            ...
                            /*
                                 ...
                                 * 
                                 */
                            // vi kör den för längden på ordlistans elementen minus en
                            for (int i = 0; i < ordLista.Count - 1; i++)
                            {
                                // denna körs med samma längde men kortas med ytterliggare en som ökas efter första omgången
                                for (int j = 0; j < ordLista.Count - 1 - i; j++)
                                {
                                    // vi jämför första bokstäver på elementen i ordlistans elementen och sparar resultatet som är en siffra
                    ...
                    case 6: // Här ska användaren kunna söka på första bokstaven av våra nonsensord i ordLista.                                                      (BINÄRSÖKNING AV BOKSTÄVER)
                        ...
                        if (ordLista.Count > 0)
                        {
                            if (bokstavSortering)
                            {
                                ...
                                int first = 0;
                                int last = ordLista.Count - 1;
                                while ( first <= last)
                                {
                                    // vi räknar ut mittnummer av element listan
                                    int mid = (first + last) / 2;
                                    // vi jämför nyckel bokstav med första bokstav i ordet i mitten av listan
                                    if (key.CompareTo(ordLista[mid].Substring(0, 1)) > 0)
                                         first = mid + 1;
                                    else if (key.CompareTo(ordLista[mid].Substring(0, 1)) < 0)
                                        last = mid - 1;
                                    else
                                    {
                                        Console.WriteLine("\n\tSöknings bokstav {0} finns på plats nr {1}?", key, mid + 1);
                                        // jag har lagt till break för att while fortsätter om man söker på ett nummer
                                        // som är direkt i mitten i början, då är first < last och while-loop fortsätter för alltid
                                        break;
                                    }
                                ...
                                if (first > last)
                                    Console.WriteLine("\n\tSökning på bokstaven {0} misslyckades", key);
